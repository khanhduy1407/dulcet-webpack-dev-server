...
const webpack = require("@dulcetjs/webpack");
const webpackDevMiddleware = require("dulcet-webpack-dev-middleware");
...
function Server(compiler, options) {
	...
	app.get("/__dulcet-webpack_dev_server__/live.bundle.js", (req, res) => {
		res.setHeader("Content-Type", "application/javascript");
		fs.createReadStream(path.join(__dirname, "..", "client", "live.bundle.js")).pipe(res);
	});

	app.get("/__dulcet-webpack_dev_server__/sockjs.bundle.js", (req, res) => {
		res.setHeader("Content-Type", "application/javascript");
	...
	app.get("/dulcet-webpack-dev-server/*", (req, res) => {
			...
			content.forEach(function(item) {
				...
				if(fs.statSync(p).isFile()) {
					...
					if(/\.js$/.test(item)) {
						...
						res.write(item);
						res.write(') (<a href="');
						res.write(baseUrl.replace(/(^(https?:\/\/[^\/]+)?\/)/, "$1dulcet-webpack-dev-server/") + htmlItem);
						res.write('">dulcet-webpack-dev-server</a>)</li>');
					}
				} else {
					res.write('<li>');
					res.write(item);
...
Server.prototype.listen = function(port, hostname) {
	...
	const sockServer = sockjs.createServer({
		...
		sockjs_url: "/__dulcet-webpack_dev_server__/sockjs.bundle.js",
		// Limit useless logs
